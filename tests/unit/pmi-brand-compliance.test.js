...
describe('PMI brand: clickable elements use Violet (--primary), not Aqua (--accent)', () => {
  test('hotspot pin (clickable) is Violet, not Aqua', () => {
  ...
  test('tabs are complete Violet capsules — outlined at rest, filled when active — not underline tabs', () => {
  ...
  test('selected quiz option (multiple choice) uses a Violet border, not an Aqua tint fill', () => {
  ...
  test('active sorting target button is a Violet-filled capsule, not an Aqua rectangle', () => {
  ...
  test('active vertical-timeline step marker is Violet, not Aqua', () => {
  ...
  test('active horizontal-timeline node marker and label are Violet, not Aqua', () => {
  ...
  test('video overlay play button is Violet (resting and hover), not Aqua or a neutral wash', () => {
  ...
  test('pricing action button is a Violet capsule (full var(--button-radius)), not a partially-rounded rectangle', () => {
  ...
  test('clickable menu-drawer index number and arrow are Violet at rest, not just once expanded', () => {
  ...
  test('accordion indicator icons (chevron and plus-minus) are Violet at rest, not just once expanded', () => {
  ...
  test('sorting target buttons are Violet at rest, not just on hover/active', () => {
    const css = sortingActivity.generateCSS();
  ...
  test('horizontal-timeline node markers are Violet-outlined at rest, matching the vertical timeline', () => {
    const css = horizontalTimeline.generateCSS();
  ...
  test('active flip-card study filter/classify controls are Violet, not Aqua', () => {
...
describe('PMI brand: every Aqua text node is kept blue by growing to >=19px, not desaturated', () => {
  ...
  test('shared block-label eyebrow (every component\'s header) is Aqua at >=19px', () => {
  ...
  test('sorting-activity column header is Aqua at >=19px', () => {
    assertBlueAtCompliantSize(sortingActivity.generateCSS().match(/\.column-header\s*{[^}]*}/)[0]);
  });

  test('process-flow step duration line is Aqua at >=19px', () => {
    assertBlueAtCompliantSize(processFlow.generateCSS().match(/\.process-step-duration\s*{[^}]*}/)[0]);
  });

  test('video mini play/pause label (clickable, 11px) uses Violet, which carries no size restriction', () => {
    const css = videoFrame.generateCSS();
    expect(css).toMatch(/\.video-mini-play\s*{[^}]*color:\s*var\(--primary\)/);
  });
});

describe('PMI brand: Violet is reserved for clickable elements, never a passive popup surface', () => {
  test('hotspot tooltip (passive content, no control inside it) is not Violet-colored', () => {
    const css = hotspots.generateCSS();
  ...
  test('hotspot pin trigger itself is Violet (it is the clickable control)', () => {
    const css = hotspots.generateCSS();
    expect(css).toMatch(/\.hotspot-pin\s*{[^}]*background-color:\s*var\(--primary\)/);
  });
});

describe('PMI brand: no improvised tints/shades of the brand blues', () => {
  ...
  test('info-grid fallback icon accent dots reference the theme token, not a hardcoded #00799E literal', () => {
    const html = infoGrid.generateHTML({ items: [{ title: 'Card', content: 'Desc' }] });
    expect(html).not.toContain('#00799E');
  ...
  test('image-gallery caption overlay and lightbox text carry no hardcoded slate hex literal', () => {
    ...
    expect(css).not.toMatch(/#CFCBC2/i);
  });
});

describe('PMI brand: WCAG contrast holds for every retargeted color pairing (computed, not eyeballed)', () => {
  const tokens = resolveThemeTokens(getBuiltInTheme());

  test('white text on Violet (--on-primary on --primary) clears 4.5:1 at any size', () => {
  ...
  test('success and danger feedback pairings clear 4.5:1 contrast', () => {
    // PMI's status colours (Green 500, Red 500) are used as text/borders on white, so each
    // must clear 4.5:1 there on its own.
  ...
  test('Aqua (--accent) on white clears 4.5:1, so accent text needs no size workaround; the bright Aqua 300 does not and stays decorative', () => {
    expect(contrastRatio(tokens.accent, tokens.background)).toBeGreaterThanOrEqual(4.5);
    // Aqua 300 (#05BFE0) is PMI's logo colour but only 2.2:1 on white: never text.
    expect(contrastRatio('#05BFE0', tokens.background)).toBeLessThan(3);
  });
});

describe('PMI brand: components outside the original 16-slide audit, swept for the same rules', () => {
  test('multiple-select selected option is Violet-bordered, not an Aqua tint fill (matches multiple-choice)', () => {
  ...
  test('audio-player default icon has no hardcoded #00799E literal, and the play button stays Violet while playing', () => {
    ...
    // now stay within the Violet family (--primary / --primary-hover) always.
    expect(js).not.toContain("'var(--accent)'");
    expect(js).toContain("'var(--primary-hover)'");
  });

  test('audio-player speed/skip/mute controls are Violet-outlined capsules at rest, not invented tints', () => {
  ...
  test('video-frame speed/skip/mute controls are Violet-outlined at rest, not invented tints', () => {
    const css = videoFrame.generateCSS();
  ...
  test('scenario speaker name is Aqua at >=19px, and choice buttons are Violet at rest not just on hover', () => {
  ...
  test('interactive-video multiple-choice selection and marker states are Violet/success/danger tokens, no invented tints or hardcoded hex', () => {
...
describe('PMI brand: new authoring features (Slides 6 and 10)', () => {
...
describe('Prompt 3: PMI brand color tokenization and absence of off-brand literals', () => {
  ...
  test('component CSS does not contain raw off-brand color literals', () => {
    ...
    for (const comp of componentGenerators) {
      ...
      expect(css).not.toMatch(/background-color:\s*#200F3B;/);
    }
  });
...
describe('Prompt 4: PMI Typography Standards (Type Hierarchy, 16px Body Floor, 70ch Prose, 44px Touch Targets)', () => {
  const allComponents = [
